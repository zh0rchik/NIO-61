...
uint32_t make_response(int16_t value, uint16_t vmax, uint8_t digits, uint16_t addr) {
    ...
    double abs_value = (double) (is_negative ? - (long)value : (long) value);

    if (digits == 0 || digits > 27) return 0;
    addr &= 0x1FFF;

    /* Веса как double: Vmax, Vmax/2, ... */
    double weights[32];
    ...
    /* Жадный алгоритм разложения (double) */
    uint32_t pattern = 0;
    double remaining = abs_value;
    for (uint8_t i = 0; i < digits; ++i) {
        if (remaining + EPS >= weights[i]) {
            pattern |= (1u << (digits - 1 - i));
            remaining -= weights[i];
        }
    }

    /* Для отрицательного — переводим pattern в two's complement (в пределах digits бит) */
    if (is_negative) {
        uint32_t mask = ((1u << digits) - 1u);
        pattern = ((~pattern) + 1u) & mask;
        resp |= (1u << 28); /* S = 1 */
    }

    /* Кладём pattern в биты 27..(27-digits+1) */
    for (uint8_t i = 0; i < digits; ++i) {
